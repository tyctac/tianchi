...
file_suffix = '.csv'
path_old = '../'  # set the data directory
# path = config.get_home_dir()
path = './'
...
def avgTravelTime(in_file):
    ...
    out_file_2 = in_file.split("_")[1]+out_suffix+'_2'+file_suffix
    ...
    fw2 = open(out_file_name,'w')
    fw2.writelines(','.join(['"intersection_id"', '"tollgate_id"', '"time_window"', '"avg_travel_time"']) + '\n')
    ...
    for route in travel_times.keys():
        ...
        for time_window_start in route_time_windows:
            ...
            out_line = ','.join(['"' + route.split('-')[0] + '"', '"' + route.split('-')[1] + '"',
                                 ...
                                 '"' + str(avg_tt) + '"']) + '\n'
            ## －＞根据时间范围过滤记录
            if time_window_start.hour >= 8 and time_window_start.hour <10 or time_window_start.hour >=17 and time_window_start.hour < 19:
                out_line2 = ','.join(['"' + route.split('-')[0] + '"', '"' + route.split('-')[1] + '"',
                                     '"[' + str(time_window_start) + ',' + str(time_window_end) + ')"',
                                     '"' + str(avg_tt) + '"']) + '\n'
